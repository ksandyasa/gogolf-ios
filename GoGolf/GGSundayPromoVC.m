...
#import "GGSundayPromoVC.h"

@implementation GGSundayPromoVC
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([[GGCommonFunc sharedInstance] connected]) {
        ...
        [[GGAPICourseManager sharedInstance] getWeekendCourseWithDay:@"this_sunday" completion:^(NSDictionary * responseDict, NSError *error) {
            ...
            if (error == nil) {
                if ([[responseDict objectForKey:@"code"] intValue] == 400) {
                    ...
                    self.golfArr = responseDict;
                    [self.tblSundayPromotion reloadData];
                }
            } else {
...
- (void) reloadSearchTable {
    [self.tblSundayPromotion reloadData];
    [self.tblSundayPromotion setContentOffset:CGPointZero animated:YES];
}
...
- (IBAction) goBookingPromo:(id)sender {
    ...
    CGPoint buttonPosition = [sender convertPoint:CGPointZero toView:self.tblSundayPromotion];
    NSIndexPath *indexPath = [self.tblSundayPromotion indexPathForRowAtPoint:buttonPosition];
...
//        [bookPromoView setDetailArr:[[self.golfArr objectForKey:@"data"] objectAtIndex:indexPath.row]];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return [[self.golfArr objectForKey:@"data"] count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *reuseIdentifier = @"sundayPromoCell";
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:reuseIdentifier];
    
    UIImageView *imgRow = (UIImageView *)[cell viewWithTag:1];
    if([[[self.golfArr objectForKey:@"data"] objectAtIndex:indexPath.row] objectForKey:@"image"] != (id)[NSNull null])
        [[GGCommonFunc sharedInstance] setLazyLoadImage:imgRow urlString:[[[self.golfArr objectForKey:@"data"] objectAtIndex:indexPath.row] objectForKey:@"image"]];
    else
        imgRow.image = [UIImage imageNamed:@"def_big_img"];
    
    UILabel *lblTitle = (UILabel *)[cell viewWithTag:2];
    if (self.golfArr[@"data"][indexPath.row][@"gname"] == nil || self.golfArr[@"data"][indexPath.row][@"gname"] == (id) [NSNull null]) {
        lblTitle.text = @"";
    } else {
        lblTitle.text = self.golfArr[@"data"][indexPath.row][@"gname"];
    }
    
    UILabel *lblArea = (UILabel *)[cell viewWithTag:9];
    if (self.golfArr[@"data"][indexPath.row][@"area_name"] == nil || self.golfArr[@"data"][indexPath.row][@"area_name"] == (id) [NSNull null]) {
        lblArea.text = @"";
    } else {
        lblArea.text = self.golfArr[@"data"][indexPath.row][@"area_name"];
    }
    
    UILabel *lblLimit = (UILabel *)[cell viewWithTag:4];
    lblLimit.layer.cornerRadius = 5.0;
    lblLimit.layer.masksToBounds = true;
    if (self.golfArr[@"data"][indexPath.row][@"limit_num"] == nil || self.golfArr[@"data"][indexPath.row][@"limit_num"] == (id) [NSNull null]) {
        lblLimit.text = @"Limit : 0 Flight";
    } else {
        lblLimit.text = [NSString stringWithFormat:@"Limit : %ld Flight", [self.golfArr[@"data"][indexPath.row][@"limit_num"] integerValue]];
    }
    
    UILabel *lblDate = (UILabel *)[cell viewWithTag:5];
    if (self.golfArr[@"data"][indexPath.row][@"date"] == nil || self.golfArr[@"data"][indexPath.row][@"date"] == (id) [NSNull null]) {
        lblDate.text = @"<not available>";
    } else {
        lblDate.text = [[GGCommonFunc sharedInstance] changeDateFormat:self.golfArr[@"data"][indexPath.row][@"date"]];
    }
